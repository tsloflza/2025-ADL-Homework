...
output_dir = "./output/inference/v1"
...
for b_start in tqdm(range(0, len(tests), BATCH_Q)):
    ...
    for b, (q, (low, high)) in enumerate(zip(queries, idx_slices)):
        ...
        current_mrr = mrr_at_k(reranked_pids, gold_sets[b], TOP_K)
        MRR_sum += current_mrr
        batch_mrr_scores[b] = current_mrr
...
# =========================
# 6. 分析 MRR 與 Diag_Scores 的關聯
# =========================

print("\n--- Analyzing MRR vs. Similarity ---")

mrr_list = [record.get("mrr_at_k", 0.0) for record in output_records]

if len(mrr_list) != len(diag_scores):
    print(f"Warning: MRR list ({len(mrr_list)}) and Diag scores ({len(diag_scores)}) length mismatch.")

def get_distribution_stats(scores_list):
    """
    計算分數列表的統計數據，包括平均值、標準差以及在 -1 到 1 之間
    以 0.1 為間隔的直方圖分佈。
    """
    if not scores_list:
        return {
    ...
    mean_val = float(np.mean(scores_np))
    std_val = float(np.std(scores_np))

    # 3. 計算 -1 到 1 之間，以 0.1 為間隔的數量
    bin_edges = np.linspace(-1.0, 1.0, num=21)
    counts, _ = np.histogram(scores_np, bins=bin_edges)
    ...
    for i in range(len(counts)):
        ...
        # 標籤格式： "(-0.9, -0.8]"
        # np.histogram 的第一個 bin 包含左邊界
        if i == 0:
            label = f"[{left_edge:.1f}, {right_edge:.1f}]"
        else:
            label = f"({left_edge:.1f}, {right_edge:.1f}]"
            
        histogram_bins[label] = int(counts[i]) # 轉為 python int
...
# 1. diag_scores 的總體分布
diag_scores_all_dist = get_distribution_stats(diag_scores)
print(f"Overall Diag Scores Distribution (Count: {diag_scores_all_dist['count']}):")
print(json.dumps(diag_scores_all_dist, indent=2))

# 將 MRR 和 Diag_Scores 配對
combined_scores = list(zip(mrr_list, diag_scores))

# 2. mrr > 0.3 時，diag_scores 的分布
diag_scores_mrr_gt_03 = [ds for mrr, ds in combined_scores if mrr > 0.3]
diag_scores_mrr_gt_03_dist = get_distribution_stats(diag_scores_mrr_gt_03)
print(f"\nDiag Scores Distribution (where MRR > 0.3):")
print(json.dumps(diag_scores_mrr_gt_03_dist, indent=2))

# 3. mrr <= 0.3 時，diag_scores 的分布
diag_scores_mrr_le_03 = [ds for mrr, ds in combined_scores if mrr <= 0.3]
diag_scores_mrr_le_03_dist = get_distribution_stats(diag_scores_mrr_le_03)
print(f"\nDiag Scores Distribution (where MRR <= 0.3):")
print(json.dumps(diag_scores_mrr_le_03_dist, indent=2))

# 儲存分析結果
analysis_results = {
    "overall_diag_scores": diag_scores_all_dist,
    "diag_scores_where_mrr_gt_0.3": diag_scores_mrr_gt_03_dist,
    "diag_scores_where_mrr_le_0.3": diag_scores_mrr_le_03_dist
}

# 儲存路徑
analysis_file_path = os.path.join(result_file_path, "mrr_vs_cos.json")
